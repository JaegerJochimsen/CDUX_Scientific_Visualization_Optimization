...
int main(int argc, char *argv[]){
    ...
    std::vector<std::vector<Vertex>> subdomain_start_basis(NUM_SUBDOMAINS);
    std::vector<std::vector<Vertex>> subdomain_end_basis(NUM_SUBDOMAINS);
    std::vector<std::vector<Vertex>> subdomain_query_points(NUM_SUBDOMAINS);
    ...
    if(doGlobal){
        ...
        for(int domId = 0; domId < NUM_SUBDOMAINS; ++domId){
